...
echo '    eval "$(oh-my-posh init bash --config ~/themes/dracula.omp.json)"' >> "$SHELL_PROFILE"
echo 'fi' >> "$SHELL_PROFILE"

# Step 4: Download the dracula theme
echo "Downloading the dracula theme..."
mkdir -p ~/themes
curl -o ~/themes/dracula.omp.json https://raw.githubusercontent.com/JanDeDobbeleer/oh-my-posh/main/themes/dracula.omp.json
...
echo "Oh My Posh with the dracula theme, Fastfetch, Yazi, and Cava have been installed successfully!"
